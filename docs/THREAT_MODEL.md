...
| Weather location leakage | Weather lookups should use user-provided locations only, avoid silent IP or device geolocation, avoid long-term location history by default, redact secrets and sensitive location details in audit/debug output where practical, and require a separate approval-gated design before using precise device location or stored home/work locations. |
| Weather provider manipulation, outage, or stale data | Treat provider responses as `UNTRUSTED_WEB`; include retrieval/provider timestamps when available; normalize provider errors; enforce timeouts and rate limits; return structured failures instead of hallucinated forecasts. |

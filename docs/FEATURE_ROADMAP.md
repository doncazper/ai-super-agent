...
Canonical Runtime Gateway Hardening is complete locally through CANON-10 plus EXTREV-01. The track now has metadata-first canonical state, durable record contracts, gateway/kernel boundary docs, recovery/checkpoint previews, artifact hash/safety lint checks, surface regression lanes, backup restore policy checks, dashboard/handoff views, tracker-sync previews, external review parity docs, release-gate docs, focused tests, full-suite evidence, command registry validation, policy-check evidence, and conservative maturity tracking. It remains local-tested scaffolding: no runtime replacement, web server, background service, live provider call, personal-data enablement, send/write enablement, package install, model download, commit, or push was added. Brain Runtime Independence completed its local BRAIN-11 release gate: architecture docs, provider-neutral interfaces, LM Studio wrapping, disabled-by-default llama.cpp server/Ollama/llama-cpp-python in-process/MLX scaffolds, mock-first benchmark/eval commands, metadata-only fallback/router decisions, disabled MCP stubs, release-gate docs, and maturity review are in place while preserving current LM Studio/Qwopus behavior. No runtime install, model download or pull, daemon/server startup, paid/cloud API default, MCP enablement, network listener, startup-heavy import, default-provider switch, or normal chat behavior change is included. LM Studio dependency is partially optional, not fully removed, because normal live chat still uses the existing LM Studio-compatible path until a future approved gateway wiring prompt. The Hermes-Inspired Safe Autonomy track is complete through HERMES-12: architecture/policy docs, safe gateway/channel process scaffolding, read-only channel metadata commands, Telegram/mobile status scaffolding, repeated-task skill proposal commands, evidence-backed skill improvement proposal commands, scheduler UX/dry-run commands, mock-only subagent isolation profiles, mock sandbox backend abstraction, model-switching/session-continuity metadata, redacted memory continuity/context-preview commands, authorized web automation boundary docs, and safe autonomy dogfood/eval suites exist; no external channel connection, bot polling, webhook/listener, runtime autonomy, browser automation, background persistence, personal-data access/carryover, send/write behavior, subagent write permission, subagent direct tool execution, self-approval, direct tool execution from channels, automatic skill creation/import/enablement/execution, automatic skill modification, lockfile update, package install, OS scheduler persistence, unattended HIGH/CRITICAL workflow, real subagent launch, parallel workflow execution, executable sandbox, persistent provider switch, paid/cloud switch default, continuity memory write, cloud embeddings, automatic prompt injection, CAPTCHA/Cloudflare/anti-bot/proxy/login/paywall bypass, stealth automation, or cloud/private-data execution was added. Release Hardening Loop v2 is complete locally with YELLOW release readiness: tests, startup policy, capability manifest, command registry, safe eval, dogfood dry-run, quality bug review, release docs, and tracker reconciliation are complete, while a clean release candidate boundary and live/manual validation remain next. News Intelligence roadmap and source policy are now specified as a docs-only track with safe manifest/provider-policy scaffolding; no runtime news commands, provider calls, paid API defaults, browser automation, search/news history storage, full article-body storage, or paywall/login/CAPTCHA bypass were added. Cross-Platform Core + Platform Bridge architecture, Platform Capability Registry v1, Platform bridge base interfaces, platform-aware config/paths/detection scaffolding, read-only platform doctor/capability commands, macOS/iOS/Windows/web bridge stubs, the App Bridge API contract, and the current cross-platform release gate/future build guides are now complete locally for the current groundwork. All future bridge actions must still route through ToolBroker, PolicyEngine, PermissionManager, ApprovalManager, and AuditLogger. Manifest mapping and dedicated startup/lazy-load guardrails remain planned follow-up prompts and must land before real platform behavior. Tracker Hygiene, Indexing, and Compaction is complete as docs-only navigation/maintenance work. Prompt Tracker Maturity track is complete locally. Agent DNA / Cloneability is complete as docs/provenance work with validation and no runtime behavior change. The Internet Access Graduation Track remains complete through local/mock-first web provider, research, cache, forum, and dogfood/release-gate work with live validation opt-in. `APPLE-MESSAGING-ROADMAP` and `apple-messaging-roadmap-track` are complete as planning-only Apple Ecosystem + Lead Response / Messaging roadmap work.
...
|---:|---|---|---|---|
| 1 | CANON-01 - Canonical runtime state model and source-of-truth hierarchy | complete | Runtime Orchestration, PromptOps, tracker hygiene | Metadata-only JSON state, hierarchy, reconcile preview, docs, commands, and tests |
| 2 | CANON-02 - Durable job, workflow, and prompt execution records | complete | CANON-01 | Record contracts, read-only record commands, validation, docs, and tests; no writer/executor/background persistence |
| 3 | CANON-03 - Agent Gateway / Runtime Kernel boundary | complete | CANON-02 | Metadata contracts, read-only status commands, docs, and tests; no server/listener/native UI/channel/tool exposure |
| 4 | CANON-04 - Resume, recovery, and checkpoint model | complete | CANON-03 | Checkpoint/recovery contracts, preview-only CLI, docs, and tests; no auto-resume or rollback |
| 5 | CANON-05 - Code Mode / self-heal artifact hash checks and safety lints | complete | CANON-04 | Hash/lint metadata commands, docs, and tests; no arbitrary script execution or auto-patching |
| 6 | CANON-06 - Surface regression lanes | complete | CANON-05 | Safe regression lane registry, dry-run commands, dogfood suites, docs, and tests |
| 7 | CANON-07 - Backup roundtrip and restore hardening lane | complete | CANON-06 | Brokered roundtrip/policy/restore-check commands, docs, and local-safe restore hardening tests complete; live restore smoke remains disposable-workspace-only |
| 8 | CANON-08 - Canonical state dashboard and docs | complete | CANON-07 | Read-only dashboard/handoff docs and commands complete; no server, provider call, tracker mutation, or file write |
| 9 | CANON-09 - Tracker-to-canonical-state migration plan | complete | CANON-08 | Plan-only tracker-sync preview/conflict commands and docs complete; no auto-overwrite or broad tracker rewrite |
| 10 | EXTREV-01 - External architecture review hardening parity checks | complete | CANON-09 | Review/parity docs/tests complete; audit receipt commands tracked as planned-only |
| 11 | CANON-10 - Canonical runtime and gateway hardening release gate | complete | EXTREV-01 | Local release gate and maturity review complete; live/manual validation and clean release-candidate boundary remain follow-ups |

## Natural-Language Command Understanding Track

This track improves terminal ergonomics by mapping loose user requests to existing commands, clarification questions, or safe preflight plans. It must remain deterministic-first, command-registry-backed, and subordinate to ToolBroker, PolicyEngine, PermissionManager, ApprovalManager, and AuditLogger.

| Order | Feature / Task | Status | Prerequisites | Approval / Gate |
|---:|---|---|---|---|
| 1 | Natural-language command understanding architecture | complete | Command Registry, prompt tracking, safety control plane | Documentation/taxonomy/safety policy only; no runtime behavior change or command execution |
| 2 | Command registry intent index | complete | NLCMD-01 | No command execution; planned/stubbed/deprecated commands not primary active suggestions |
| 3 | Natural-language request parser and deterministic router | complete | NLCMD-02 | Parser cannot call tools/providers/personal data; HIGH/CRITICAL never auto-execute |
| 4 | Clarification and confirmation flow | complete | NLCMD-03 | Ask clarifying questions or show safe command suggestions; no execution |
| 5 | Safe execution planner and natural-language preflight | complete | NLCMD-04 | Preflight only; ToolBroker/PolicyEngine/ApprovalManager remain final execution gates |
| 6 | Conversational CLI UX | complete | NLCMD-05 | Exact commands preserved; risky requests never silently execute |
| 7 | Natural-language eval fixtures | complete | NLCMD-06 | Fixture-backed; no risky execution |
| 8 | Natural-language dogfood suite and session feedback integration | complete | NLCMD-07 | Safe suite and risky preflight-only suite added with session-feedback runbook; no sends/writes/personal-data reads |
| 9 | Natural-language bug feedback loop | complete | NLCMD-08 | Redacted feedback tags, local bug records, and generated eval fixtures only; no personal data |
| 10 | Natural-language command understanding release gate | complete | NLCMD-09 | Full validation and conservative maturity review complete; advisory UX is local-tested, not autonomous execution |

## Productization Maturity Track

This track keeps release quality ahead of feature breadth. It does not add runtime capabilities; it reconciles evidence, trackers, tests, manual validation, and release gates.

| Order | Feature / Task | Status | Prerequisites | Approval / Gate |
|---:|---|---|---|---|
| 1 | Full feature status, maturity, and prompt-tracker audit | complete | Existing trackers, prompt ledger/queue/audit, command registry, dogfood/eval reports | Audit-only docs and tests; no runtime feature implementation, no live provider calls, no queued prompt execution |
| 2 | Codebase bug review and hardening | complete | MATURITY-AUDIT-01 and imported CODEBUG pack | Controlled CODEBUG-01 through CODEBUG-08 batch complete; one scoped CLI bug fixed; no major features or policy weakening; release gate passed locally |
| 3 | Command QA sandbox and self-healing loop | complete | Command registry, dogfood/evals, local bug/regression store | QA-01 through QA-10 complete with safe runner, disposable workspace, redacted reports, bug/regression scaffolds, conservative self-heal plans, service/API frontend-backend boundary, dashboard, dogfood/evals, and release gate; no high-risk autonomy enabled |
| 4 | Prompt tracker reconciliation | planned | MATURITY-AUDIT-01, CODEBUG batch, Command QA release gate | Fix stale rows and orphan/missing prompt files without deleting history; stop before destructive cleanup |
| 5 | Clean release-candidate boundary | planned | Prompt tracker reconciliation | Requires explicit review before cleanup/staging; run full tests, command validation, and policy checks |
| 6 | All-safe dogfood evidence pass | planned | Clean release-candidate boundary preferred | Mock/local only by default; no paid APIs, personal-data tools, sends/writes, or live providers without explicit setup |

## Hermes-Inspired Safe Autonomy Track

This track lays safe groundwork for selected Hermes-like ideas without enabling dangerous autonomy. It is proposal-first, disabled-by-default, no-background-persistence by default, no personal-data by default, no send/write by default, and no bypass by default. Channels, schedulers, subagents, model routing, memory continuity, and sandbox backends must remain subordinate to ToolBroker, PolicyEngine, PermissionManager, ApprovalManager, and AuditLogger.

| Order | Feature / Task | Status | Prerequisites | Approval / Gate |
|---:|---|---|---|---|
| 1 | Hermes-inspired safe autonomy architecture | complete | Brain Runtime Independence, Runtime Orchestration, Native Skill System, Scheduler v1 | Documentation-only architecture, risk model, high-risk gates, Hermes comparison, unauthorized bypass policy, tracker updates, and validation; no runtime autonomy |
| 2 | Gateway and channel process | complete | HERMES-01 | Channel-neutral models, registry, redaction, gateway guards, brokered read-only `channels` metadata commands, docs, and tests; no external channel connection, sends, self-approval, direct tool execution, or background persistence |
| 3 | Telegram and mobile access scaffolding | complete | HERMES-02 | Disabled-by-default Telegram/mobile diagnostics with brokered status commands; no bot, polling, webhook listener, mobile pairing, mobile approval executor, message reads, or message sends |
| 4 | Repeated-task skill creation proposals | complete | HERMES-03 | Brokered proposal records from redacted command/session metadata only; no automatic skill creation, import, enablement, dependency install, external script execution, or skill execution |
| 5 | Skill improvement from experience | complete | HERMES-04 | Brokered evidence-backed improvement proposals from redacted bug/dogfood metadata only; no automatic skill edits, lockfile updates, enablement, script execution, or review bypass |
| 6 | Scheduler UX for safe automations | complete | HERMES-05 | Metadata/dry-run/manual-review UX only; no hidden background runner, OS persistence, unattended HIGH/CRITICAL actions, or send/write behavior |
| 7 | Subagent isolation groundwork | complete | HERMES-06 | Mock-only profile/isolation metadata; no real subagent execution, write permissions, direct tool calls, personal-data access, or approval bypass |
| 8 | Sandbox backend abstraction | complete | HERMES-07 | Mock/default backend and planned-backend metadata; brokered dry-run policy only; no arbitrary command execution, networked sandbox, Docker/VM/browser/cloud execution, personal-data access, or broad filesystem roots |
| 9 | Model switching and session continuity | complete | HERMES-08 | Model switch records, compatibility checks, rollback metadata, and redacted continuity status/export/clear complete; non-dry-run switch requests are blocked, no silent paid/cloud fallback, no default switch, no context migration, and no memory write |
| 10 | Long-term memory search and cross-session continuity | complete | HERMES-09 | Brokered memory continuity status/build-summary/clear and context-preview complete with redaction, safe-category filtering, token limits, no cloud embeddings, no separate continuity store, no automatic injection, and no personal memory injection by default |
| 11 | Authorized web automation boundary | complete | HERMES-10 | Docs/tests/command tracking boundary only; CAPTCHA/Cloudflare/anti-bot/proxy/rate-limit/login-wall/paywall/cookie/session/human-impersonation/stealth bypass forbidden; no browser automation implemented |
| 12 | Safe autonomy dogfood and eval suite | complete | HERMES-11 | Mock/fixture-first dogfood/eval suites, runbook, command tracking, and tests; no live risky autonomy, no personal data, no external scripts, no network, no background persistence, and no HIGH/CRITICAL actions |
| 13 | Hermes-inspired safe autonomy release gate | complete | HERMES-12 complete | HERMES-13 validation, release-gate docs, and conservative maturity review complete; safe autonomy groundwork remains local-tested only and high-risk autonomy stays forbidden/deferred |

## Brain Runtime Independence Track

This track removes the hard dependency on LM Studio by introducing a provider-neutral Brain Runtime Gateway. LM Studio remains supported and is the compatibility baseline until later prompts refactor the runtime path.

| Order | Feature / Task | Status | Prerequisites | Approval / Gate |
|---:|---|---|---|---|
| 1 | Brain runtime architecture and LM Studio decoupling | complete | Agent DNA / Cloneability and current core runtime docs | Documentation-only ADR, provider strategy, LM Studio decoupling plan, MCP decision, planned command tracking, tracker updates, and validation complete; no runtime behavior change |
| 2 | BrainProvider interface and model registry | complete | Brain runtime architecture and LM Studio decoupling | Provider-neutral interface, shared models, lazy registry, deterministic mock provider, docs, and tests complete; no provider calls, LM Studio migration, or default runtime change |
| 3 | LM Studio provider refactor | complete | BrainProvider interface and model registry | `LMStudioBrainProvider`, lazy provider registration, provider-neutral response handling, metadata-only brain status/doctor/providers commands, and compatibility tests complete; LM Studio remains default |
| 4 | llama.cpp server provider | complete | LM Studio provider refactor | Disabled-by-default OpenAI-compatible server adapter, config, explicit health/status diagnostics, docs, and mocked tests complete; no install, download, server startup, or default-provider change |
| 5 | Ollama provider | complete | llama.cpp server provider | Disabled-by-default OpenAI-compatible adapter, config, explicit health/status diagnostics, docs, and mocked tests complete; no install, model pull, daemon startup, or default-provider change |
| 6 | llama-cpp-python in-process provider | complete | Ollama provider | Disabled-by-default optional dependency scaffold, dependency detection without startup import, lazy explicit model loading, docs, and mocked tests complete; no package install, model download, health/status model load, default-provider change, or real model test load |
| 7 | MLX provider strategy/stub | complete | llama-cpp-python provider | Disabled-by-default experimental stub, Apple Silicon detection metadata, docs, and tests complete; no MLX install, model download, native import, model load, server start, text generation, or default-provider change |
| 8 | Model health, benchmark, and quality evals | complete | Provider stubs/implementations | Mock-first health/benchmark/eval/report scaffolding complete; live checks remain opt-in and no paid/cloud/default-provider behavior was added |
| 9 | Provider fallback and model router | complete | Model health, benchmark, and quality evals | Metadata-only provider router, disabled-by-default fallback policy, dry-run switch, route explanation, bounded attempts, tool-call support gates, and cloud fallback denial complete; no persisted provider switch, paid/cloud fallback, or live gateway wiring |
| 10 | Optional MCP interop decision and adapters | complete | Provider fallback and model router | Decision docs and disabled-by-default MCP adapter/server/client stubs complete; no MCP package install, server/listener, external connection, tool exposure, personal tool exposure, or MCP brain-runtime requirement |
| 11 | Brain runtime release gate | complete | Optional MCP interop decision and adapters | Local release gate, maturity review, focused BRAIN/MCP tests, command registry validation, startup/capability validation, safe benchmark/eval smokes, provider diagnostics, MCP diagnostics, and optional-import guard checks complete; full provider independence remains future gateway wiring |
...
## Performance Bottleneck Scanner Track

This track adds safe, local-first performance scanning and release-hardening evidence. It must not install packages, run live providers by default, call paid APIs, download models, access personal data, start background services, broadly rewrite code, weaken safety policy, commit, or push.

| Order | Feature / Task | Status | Prerequisites | Approval / Gate |
|---:|---|---|---|---|
| 1 | Performance scanner architecture and policy | complete | Release hardening, command registry, QA sandbox | Docs, risk model, finding schema, optimization policy, baseline strategy, planned command tracking, and docs tests only; no runtime scanner or benchmark execution |
| 2 | Performance finding models and report store | complete | PERF-01 | JSON-serializable models, stable finding IDs, redacted JSON/Markdown reports under `reports/performance/`, structured no-report states, and read-only report/finding commands complete; no scanner or benchmark runtime yet |
| 3 | Static bottleneck scanner | complete | PERF-02 | Static local scan, default exclusions, heuristic patterns, redacted reports, brokered commands, and tests complete; findings require human review |
| 4 | Startup and import overhead scanner | complete | PERF-03 | Bounded subprocess timing, missing-command skips, import timing, redacted reports, brokered commands, and tests complete; no provider/model/server startup |
| 5 | Safe command benchmark runner | complete | PERF-04 | Registry-gated SAFE/LOW read-only command benchmarks, bounded subprocesses, redacted timing reports, and denial tests complete; rejects HIGH/CRITICAL/personal/live/mutating/approval-required commands |
| 6 | Test suite performance profiler | complete | PERF-05 | Local pytest `--durations` profiling, explicit full-suite opt-in, redacted profile reports, module summaries, and tests complete; no test deletion, skipping, xfail, editing, or failure hiding |
| 7 | Optimization recommendation engine | complete | PERF-06 | Advisory recommendations from redacted performance reports complete; no automatic patching or safety weakening |
| 8 | Performance baseline and regression tracking | complete | PERF-07 | Redacted local baselines and tolerance-based regression comparison complete; no history of raw prompts/provider content |
| 9 | Safe optimization patch planner | complete | PERF-08 | Metadata-only patch planning complete; broad refactors and safety-policy changes are blocked or human-review-required |
| 10 | Performance dashboard and QA integration | complete | PERF-09 | Read-only dashboard/status/next/trends summaries complete; no scans/benchmarks/tests/fixes run from dashboard views |
| 11 | Performance scanner release gate | complete | PERF-10 | Local release gate, command registry validation, startup/capability checks, safe scans/benchmarks/test profiling, baseline comparison, dashboard smokes, and maturity review complete; live/manual validation remains separate |
...
## Secrets and API Key Management Track

This track centralizes secret policy, redaction, setup diagnostics, local environment hygiene, optional Keychain strategy, leak scanning, and Git preflight behavior. It must never store real secrets in tracked files, print secret values, access Keychain without an explicit future adapter/release gate, enable paid APIs by key presence, or make provider calls from config doctors.

| Order | Feature / Task | Status | Prerequisites | Approval / Gate |
|---:|---|---|---|---|
| 1 | Secrets management architecture and policy | complete | Safety control plane, prompt tracking | Docs/policy/inventory/template work only; no real secrets or provider calls |
| 2 | Secret registry and redaction layer | complete | SECRETS-01 | Static metadata and fake-fixture redaction tests only; no real secret reads |
| 3 | Secret source resolver and local environment loader | complete | SECRETS-02 | Env/.env presence and source metadata only; values never returned |
| 4 | Provider secret doctors | complete | SECRETS-03 | Config-only provider diagnostics; no live API calls or paid-provider enablement |
| 5 | macOS Keychain strategy and optional adapter | complete | SECRETS-04 | Status/dry-run metadata only; real reads/writes blocked |
| 6 | Secret leak scanner and Git preflight | complete | SECRETS-05 | Best-effort tracked/staged scan with redacted findings; no history rewrite, commit, or push |
| 7 | Secrets documentation and user guide integration | complete | SECRETS-06 | User-facing setup, rotation, and provider mapping docs |
| 8 | Secrets management release gate | complete | SECRETS-07 | Full suite, secrets tests, command registry, policy/capability validation, scan/preflight evidence complete locally |

## Creative Media Generation Track

This track prepares image, video, audio, music, TTS, thumbnail, social creative, and future creative workflows without installing models, downloading models, calling paid APIs, generating media by default, uploading/publishing media, implementing voice cloning, or enabling real-person likeness workflows. Early milestones are docs, policy, provider metadata, stubs, mocks, asset controls, dogfood, and release gates before real generation.

| Order | Feature / Task | Status | Prerequisites | Approval / Gate |
|---:|---|---|---|---|
| 1 | Creative media roadmap and risk model | complete | Existing safety control plane, command registry, prompt tracking | Documentation-only roadmap/risk/license/asset/safety policy and planned command tracking; no runtime media generation |
| 2 | Media provider registry and output asset manager | complete | MEDIA-01 | Static/mock provider registry, bounded workspace asset metadata manager, brokered read-only/dry-run commands, and tests; no model install, provider call, or real generation |
| 3 | Media prompt/output safety, copyright, consent, and license policy | complete | MEDIA-02 | Deterministic local safety preflight, license report, consent policy, docs, and tests; no generation or external moderation API |
| 4 | ComfyUI provider strategy and stub | complete | MEDIA-03 | Stub only, disabled by default, no ComfyUI install/server start |
| 5 | Image generation provider track | complete | MEDIA-04 | Dry-run planner and mock-provider metadata only; no real generation or paid API default |
| 6 | Image editing, thumbnail, and social creative workflows | complete | MEDIA-05 | Dry-run template planning and mock asset metadata helper only; no real editing, personal-image input, or publishing |
| 7 | Video generation provider strategy | complete | MEDIA-06 | Dry-run video planner and mock-provider metadata only; no model download, runtime install, source-image input, or real generation |
| 8 | Audio, music, and sound generation strategy | complete | MEDIA-07 | Dry-run audio/music planner and mock-provider metadata only; voice cloning denied/deferred and artist imitation flagged |
| 9 | TTS and voice generation strategy | complete | MEDIA-08 | Generic TTS planned/stubbed only; voice cloning and person imitation denied/deferred until explicit consent workflow exists |
| 10 | Media workflow commands and natural-language routing | complete | MEDIA-09 | `media plan` and NL `media.plan` route to dry-run/preflight only; no automatic generation |
| 11 | Media dogfood and eval suite | complete | MEDIA-10 | Mock/fixture-only by default; four dogfood suites plus `eval --media` fixtures added |
| 12 | Creative media release gate | complete | MEDIA-11 | Local release gate and conservative maturity review complete; no live generation unless separately approved |
...
| 11 | Backup / restore / migration v1 | complete | Workspace file policy, audit review, and CANON-07 restore hardening | Brokered redacted archive create/list/inspect/verify/export plus read-only roundtrip/policy/restore checks and approval-gated restore with integrity, secret, traversal, approval-reuse, and policy-weakening guards |

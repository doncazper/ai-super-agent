...
- `tests/performance/test_performance_docs.py` verifies PERF-01 docs, safety boundaries, planned commands, and bottleneck categories.
- `tests/performance/test_performance_models_reports.py` verifies JSON-serializable models, stable finding IDs, redacted report writes, no-report states, latest findings, and tool registration.
- `tests/performance/test_static_bottleneck_scanner.py` verifies safe static source scanning, default exclusions, heuristic pattern detection, redacted report writes, and no code execution.
- `tests/performance/test_startup_scanner.py` verifies bounded subprocess startup/import timing, missing-command skips, timeout handling, report payloads, and tool registration.
- `tests/performance/test_benchmark_runner.py` verifies registry-gated command benchmarks, unknown command denial, HIGH/CRITICAL and placeholder denial, median/min/max timing metadata, raw output omission, redacted report writes, and tool registration.
- `tests/performance/test_test_profiler.py` verifies pytest duration parsing, safe target profiling, explicit full-suite opt-in, outside-target denial, redacted profile report writes, latest profile reads, and tool registration.
- `tests/performance/test_optimization_recommendations.py` verifies finding-to-recommendation mapping, architecture-review fallback, pytest-profile recommendations, no-report behavior, applied-patches-zero behavior, and tool registration.
- `tests/performance/test_performance_baselines.py` verifies baseline creation, branch/commit/Python/platform metadata, tolerance-based regression ranking, finding-count regression, no-report/no-baseline states, and tool registration.
- `tests/performance/test_performance_patch_planner.py` verifies patch-plan allow/block gating, broad/safety-sensitive review behavior, report writes, recommendation filtering, no-recommendations state, applied-patches-zero behavior, and tool registration.
- `tests/performance/test_performance_dashboard.py` verifies dashboard/status/next/trends read-only summaries, report aggregation, recommendations, patch-plan candidates, and tool registration.
- PERF-11 release gate reruns focused performance tests, command registry validation, startup/capability validation, safe local scans/benchmarks/test profiling, recommendation generation, baseline comparison, and dashboard/status smokes.
- Future PERF prompts must add focused tests for command-registry scanning beyond current safe command benchmarks.
- Safe benchmark tests must prove HIGH/CRITICAL, personal-data, mutating, live-provider, paid-provider, background-service, and broad-refactor paths are rejected by default.
...
- Natural-language command understanding docs tests verify the NLCMD-01 track docs, intent taxonomy, safety outcomes, and decision record exist and name ToolBroker, PolicyEngine, ApprovalManager, and AuditLogger as non-bypassable boundaries.
- Command intent index tests verify the natural-language metadata index builds from fixture command records, active commands can be suggested, deprecated commands are not primary, stubbed commands are labeled, provider-required commands show setup hints, and HIGH/CRITICAL commands are dry-run/approval-only rather than directly runnable.
- Natural-language clarification flow tests verify missing required arguments, ambiguous command matches, provider/setup hints, deprecated replacements, personal-data approval/setup requirements, risky send/write no-execution previews, and router regressions.
- Natural-language preflight tests verify safe weather plans, web provider/setup requirements, email send approval denial, file write approval/preview, unknown denial/help, no tool execution metadata, and explain/suggest outputs.
- Natural-language CLI UX tests verify exact command dispatch remains intact, `nl` and `ask` explanation surfaces map common requests, risky requests are not executed, ambiguity asks for clarification, no-tools mode is preserved, and help text advertises the safe NL subcommands.
- Natural-language eval fixture tests verify fixture loading, safe cases, risky no-execution metadata, ambiguous clarification, unsupported help/missing-intent behavior, and command registry entries for `eval run/report --natural-language`.
- Natural-language dogfood suite tests verify `natural_language_core` and `natural_language_risky` suite YAML loading, required realistic phrases, risky preflight-only constraints, no direct send/write/personal-data/prompt execution commands, and command registry entries for the suite run commands.
- Natural-language bug feedback loop tests verify NL feedback tags, `feedback nl-bug` redacted bug creation, personal-data/secret redaction, `bugs create-nl-regression` sanitized fixture generation, eval-case loading, `nl regressions list`, and command registry entries.
- Natural-language parser/router tests verify weather, vague lookup, command-help, session-bug, send/write, ambiguous, no-tools, exact-command, and high-risk requests return structured route decisions without executing commands or tools.
- Command QA sandbox and self-healing tests verify registry-backed command inventory, tier classification, safe-only planning, runner shell-marker and placeholder blocking, timeout/redaction/report writing, disposable QA workspace init/status/clean, Tier 3 sandbox enforcement, bug ranking and redacted bug generation, skipped regression scaffolding, self-heal patch-plan gating, P0/P1 human-review requirements, progressive dry-run schedules, service-backed dashboard/status/next-fix summaries, frontend/backend API boundary behavior, dogfood suite YAML, and fixture-backed command QA evals without automatic HIGH/CRITICAL execution, personal-data access, package installs, commits, pushes, or auto-patching.
- Productization audit docs tests verify the full feature maturity audit, scorecard, missed-prompts audit, next maturity queue, next feature expansion candidates, and manual validation plan exist, keep live/provider work opt-in, name stale prompt tracker evidence, and include conservative maturity caveats.
...
- Hermes-inspired safe autonomy release-gate tests verify gateway/channel, Telegram/mobile, skill proposal, skill improvement, scheduler UX, subagent isolation, sandbox abstraction, model switching, memory continuity, authorized web boundary, dogfood/eval, command registry, startup policy, and capability manifest checks remain local, fixture/mock/dry-run, brokered where executable, disabled-by-default where risky, and conservative in feature maturity.
...
- Codebase bug review regression tests cover closed-pipe CLI output handling so `smart_agent.py commands list | head -5` exits without a `BrokenPipeError` traceback.
...
- Safe autonomy dogfood/eval tests cover safe autonomy core, channel gateway, Telegram/mobile, skill proposal, subagent isolation, sandbox abstraction, memory continuity, and authorized web boundary suite YAML validation; fixture-backed `eval run --safe-autonomy` checks disabled defaults, proposal-only behavior, scheduler dry-runs with no tool execution, mock-only subagents/sandbox, model-switch dry-run with no paid/cloud calls, personal-data-excluding continuity, blocked web automation boundaries, no external scripts, no HIGH/CRITICAL action execution, CLI flag parsing, and command registry entries.
